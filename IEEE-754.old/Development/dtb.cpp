//dtb covert a decimal number to a binary number
#include <iostream>
#include <string>
#include <vector>
#include <gmp.h>
...
int main(int argc, char* argv[]){
	...
	mpz_init(result);
      ...
      while (decimal_exponent > 0)//adjust the working copy so that the exponent is zero
      {
	...
	mpz_set_str(decimal_integer, di.c_str(), 10);//create local copy
	...
	if ((atoi((decimal_recurrence.c_str())) > -1) && (decimal_fraction.length() < decimal_recurrence.length()))
	{
	...
	mpf_t powToMul;//this variable is the power to multiply the superR by in order to get the decimal point to the right of the recurrence spot
	mpf_init(powToMul);
	...
	superRL = mpf_get_ui(superR);//this should do away with the decimal stuff leaving just the numbers to the left of the decimal point
	superRL = superRL + 1;
	...
	a++;
	mpf_add(df, df, df);
	mpf_t poft; //this is the power of ten variable, only used when there are decimal recurrences
	mpf_init(poft);
	int lofr = 0; //this int variable is the length of the decimal recurrence
	...
	mpf_set(poft, d);
	//the variable drs tells us this, 0 is right next to the decimal, 1 is 1 away from this decimal and so on
	int drs = decimal_recurrence_start;//need to locate the decimal recurrence string
	//now we need to further move the decimal point to account for the distance the recurrence starts from the decimal point
	for(int i =0 ; i<drs; i++){
		mpf_div_ui(poft, poft, 10);//divide the poft by 10 for every spot the recurrence is away from the decimal point example if drs is 1 you do poft/10 one time so 0.1 would become 0.01
	}

	//now we need the power of ten that we will be comparing the "sum" to, to decide whether to add anything to the "sum"
	//this is based on the drs, if drs = 0 then we compare to 1.0, if drs = 1 we compare to .1
	mpf_t e;
	mpf_init(e);
	mpf_set_ui(e, 1);
	for(int i = 0; i< drs; i++){
	...
	if(decimal_recurrence_start> -1){ //so do this only for recurring numbers 
	...
	while((mpf_cmp_ui(df, 0)>0) && (binary_recurrence_start==-1)){
